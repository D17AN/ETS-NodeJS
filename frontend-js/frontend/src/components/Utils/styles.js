const bookOfStyles = {
    orderItem: [
      'bg-white',
      'px-4',
      'py-3',
      'gap-x-4',
      'sm:border-b',
      'sm:border-gray-200',
      'flex-1',
      'grid',
      'grid-cols-7',
      'grid-gap-1'
    ],
    orderItemId: ['text-lg', 'font-medium', 'text-gray-900', 'flex-1'],
    orderItemTitle: ['text-lg', 'font-medium', 'text-gray-900', 'flex-1', 'hidden', 'md:flex'],
    orderItemQuantity: [
      'w-[50px]',
      'text-center',
      'py-1',
      'px-2',
      'border',
      'border-orange-700',
      'border-2',
      'disabled:border-0',
      'rounded',
      'text-orange-700',
      'text-sm',
      'leading-tight',
      'font-bold',
      'disabled:text-gray-700',
      'focus:outline-none',
      'focus:shadow-outline',
    ],
    orderItemQuantityWrapper: ['flex-1', 'flex-row', 'justify-end', 'flex-1'],
    orderItemType: [
      'w-fit',
      'py-1',
      'px-2',
      'border',
      'border-orange-700',
      'border-2',
      'py-px',
      'disabled:border-transparent',
      'disabled:appearance-none',
      'disabled:text-gray-900',
      'disabled:border-2',
      'disabled:pl-3',
      'rounded',
      'leading-tight',
      'focus:outline-none',
      'focus:shadow-outline',
      'text-sm',
      'font-bold',
      'text-orange-700',
      'flex-1',
    ],
    orderItemTypeWrapper: ['flex-1', 'flex-row', 'justify-end', 'flex-1'],
    orderItemDate: ['text-center', 'flex-1', 'hidden', 'md:flex'],
    orderItemPrice: ['text-center', 'w-12', 'hidden', 'md:flex'],
    actions: ['sm:mt-0', 'sm:text-right', 'w-28'],
    actionButton: [
      'ml-2',
      'text-xl',
      'ps-2',
      'font-medium',
      'underline',
      'text-gray-700',
      'flex-1',
    ],
    deleteButton: ['hover:text-red-500'],
    cancelButton: ['hover:text-red-500'],
    saveButton: ['hover:text-green-500'],
    editButton: ['hover:text-blue-500'],
    hiddenButton: ['hidden'],
  
    eventWrapper: [
      'event-card',
      'bg-white',
      'rounded',
      'shadow-md',
      'p-4',
      'flex',
      'flex-col',
      'm-6',
      'mt-8',
      'width-500',
    ],
  
    actionsWrapper: ['actions', 'flex', 'items-center', 'mt-4'],
  
    quantity: ['actions', 'flex', 'items-center', 'mt-4'],
  
    input: [
      'input',
      'w-16',
      'text-center',
      'border',
      'border-gray-300',
      'rounded',
      'py-2',
      'px-3',
      'text-gray-700',
      'focus:outline-none',
      'focus:shadow-outline',
    ],
  
    quantityActions: ['quantity-actions', 'flex', 'space-x-2', 'ml-6'],
  
    increaseBtn: [
      'increase',
      'px-3',
      'py-1',
      'rounded',
      'add-btn',
      'text-white',
      'hover:bg-red-300',
      'focus:outline-none',
      'focus:shadow-outline',
    ],
    decreaseBtn: [
      'decrease',
      'px-3',
      'py-1',
      'rounded',
      'bg-black',
      'text-white',
      'hover:bg-black-300',
      'focus:outline-none',
      'focus:shadow-outline',
    ],
    addToCartBtn: [
      'add-to-cart-btn',
      'px-4',
      'py-2',
      'rounded',
      'text-white',
      'font-bold',
      'disabled:opacity-50',
      'disabled:cursor-not-allowed',
      'focus:outline-none',
      'focus:shadow-outline',
    ],
  };
  
  export function useStyle(type) {
    if (typeof type === 'string') return bookOfStyles[type];
    else {
      const allStyles = type.map((t) => bookOfStyles[t]);
      return allStyles.flat();
    }
  }
  
  